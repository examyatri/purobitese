  ...
  if (/^\d{4}-\d{2}-\d{2}T/.test(s)) {
    ...
    if (!isNaN(ist.getTime()))
      return String(ist.getUTCDate()).padStart(2, '0') + '/' +
        String(ist.getUTCMonth() + 1).padStart(2, '0') + '/' + ist.getUTCFullYear();
  }
  return s;
}
...
async function signupUser(data) {
  ...
  const { data: user, error } = await supabase.from('users').insert({
    name: data.name, phone: ph, email: data.email || '', address: data.address || '', password: hashed
  }).select().single();
...
async function createOrder(data) {
  ...
  if (data.userType === 'subscriber' && data.payFromWallet) {
    await deductWalletBalance(cleanPhone(data.phone), Number(data.finalAmount) || 0, 'Order ' + order.order_id);
  }
...
async function createStaff(data) {
  ...
  const hashed = await bcrypt.hash(String(data.password), 10);
  const { error } = await supabase.from('staff').insert({ username: data.username, name: data.name, password: hashed, status: 'active' });
  if (error) throw new Error(error.message);
  return true;
}
...
async function deductWalletBalance(phone, amount, note, userId) {
  ...
  const ist = getIST();
  await supabase.from('khata_transactions').insert({ user_id: userId || null, amount: -amount, type: 'debit', note: note || '', created_at: ist.toISOString() });
  return newBal;
}
...
async function rechargeWallet(data) {
  ...
  await supabase.from('khata_transactions').insert({ user_id: user?.user_id || null, amount: amt, type: 'credit', note: data.note || 'Recharge', created_at: ist.toISOString() });
...
async function addKhataEntry(data) {
  ...
  const { data: user } = await supabase.from('users').select('user_id').eq('phone', ph).maybeSingle();
  const ist = getIST();
  await supabase.from('khata_transactions').insert({ user_id: user?.user_id || null, amount, type: amount >= 0 ? 'credit' : 'debit', note: data.note || '', created_at: ist.toISOString() });
  return { newBalance: newBal };
}
async function getKhata(data) {
  ...
  const entries = txns.map(t => {
    ...
    return { entryId: t.id, phone: ph, type: t.type === 'credit' ? 'recharge' : 'tiffin_given', amount: Number(t.amount), note: t.note || '', runningBalance: running, date: istDateStr(ist), time: istTimeStr(ist) };
  });
...
async function adminGetAllKhata() {
  ...
  const balMap = {};
  (wallets || []).forEach(w => { balMap[w.user_phone] = Number(w.balance) || 0; });
  return (subs || []).map(s => ({ phone: s.phone, name: s.name || '', balance: balMap[s.phone] || 0, entries: [] }));
}
...
async function getAnalytics() {
  ...
  return { todayOrders: todayOrders.length, todayRevenue, monthlyRevenue, totalOrders: (orders || []).length, totalUsers: (users || []).length, totalSubscribers: (subs || []).length, totalWallet };
}
